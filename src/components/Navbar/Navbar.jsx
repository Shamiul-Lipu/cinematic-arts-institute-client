...
const Navbar = () => {
  ...
  const navItems = (
    <>
      <ul
        className={`flex flex-col md:flex-row justify-start md:justify-center items-start md:items-center gap-0 md:gap-4 `}
      >
        <li>
          <NavLink
            to="/"
  ...
  return (
    <header className="bg-sky-50 py-4 fixed w-full z-10 shadow-md customDark">
      <Container>
        <div className="flex flex-col md:flex-row justify-between">
          {/* logo & brand name */}
          <div className="flex gap-2 mx-auto md:ml-0  items-center">
            <Link to="/">
              <img className="w-16 py-1" src={mainLogo} alt="logo" />
            </Link>
            <Link to="/">
              <h3 className=" text-sky-600 text-2xl font-semibold">
                Cinematic{" "}
                <span className="customDark font-extrabold text-indigo-900 ">
                  Arts Institute
                </span>
              </h3>
            </Link>
          ...
          <nav className="flex gap-2 justify-between md:justify-evenly items-start md:items-center">
            ...
            <div>
              <div className="dropdown dropdown-end">
                <div className="flex justify-center items-center bg-sky-100 pl-2 rounded-3xl ">
                  {
                    <button onClick={() => setToggle(!toggle)} className="p-1">
                      {toggle ? (
                        ...
                        <FiMoon className="w-9  h-9 mx-1 text-white rounded-full bg-indigo-950 border-[3px] border-indigo-200" />
                      )}
                    </button>
                  }
                  ...
                  {user && role ? (
                    role === "student" ? (
                      <Link to={"/dashboard/my-selected-classes"}>
                        <SecondaryBtn label={"Dashboard"}></SecondaryBtn>
                      </Link>
                    ) : role === "admin" ? (
                  ...
                  <label
                    ...
                    data-tooltip-content={`${
                      user ? user?.displayName : "Sign up/Login"
                    }`}
                  ...
                  >
                    ...
                    >
                      {user ? (
                        ...
                        <Lottie className="" animationData={avatar} />
                      )}
                    </div>
                  </label>
                </div>
                ...
                <ul
                  tabIndex={0}
                  className="menu menu-sm dropdown-content mt-3 p-2 shadow  rounded-box w-52 bg-indigo-50 "
                >
                    ...
                    {user && role ? (
                      role === "student" ? (
                        <Link
                          ...
                          className="justify-between hover:bg-sky-200 py-3"
                        >
                          Dashboard
                        </Link>
                      ) : role === "admin" ? (
                  ...
                  <>
                    {user ? (
                      ...
                      <Link to="/login">
                        <PrimaryBtn label={"Login"} className=""></PrimaryBtn>
                      </Link>
                    )}
                  </>
                </ul>
