...
const ClassCard = ({ classData }) => {
  ...
  const handleSelectClass = (classData) => {
    if (user && user.email) {
      ...
      axios
        .post(`${import.meta.env.VITE_API_URL}/selected-classes`, selectedClass)
        .then((res) => {
  ...
  return (
    <div
      className={`card card-compact ${
        (classData && classData.classStatus === "pending") ||
        classData.classStatus === "denied"
          ? "hidden"
          : ""
      } ${
        classData && classData.availableSeats <= 0 ? "bg-red-200" : "bg-lime-50"
      }  shadow-xl`}
    >
      {classData && classData.availableSeats <= 0 && (
        <div className="badge badge-error gap-2 h-7 rounded-xl text-lg w-full rounded-b-none font-semibold text-red-100">
      ...
      <figure>
        <img
          ...
          src={classData.imgUrl}
          alt="Shoes"
        />
      </figure>
      <div className="card-body textBlack">
        <h2 className="card-title font-bold"> {classData.class}</h2>
        {classData?.classRating && (
          <p className="flex justify-start items-center">
            <span className="text-yellow-400">
              <FaStar />
            </span>{" "}
            <span className="text-xs">{classData?.classRating} out of 5 </span>
          </p>
        )}
        <p className="font-bold text-lg ">Instructor: {classData.instructor}</p>
        <p className="font-semibold text-base text-green-950">
          Price: {classData.price}$
        </p>
        <p
          className={`font-semibold text-base ${
            classData?.availableSeats && classData.availableSeats >= 1
              ? "text-lime-600"
              : "text-red-800 py-1 px-2 rounded-md bg-orange-100 w-3/6"
          }`}
        >
          Available Seats: {classData.availableSeats}
        </p>
        <p className="font-semibold text-base">
          Number of student: {classData.numOfStudent}
        </p>
        <p className="font-normal text-sm">
          {classData?.details?.slice(0, 150)}
          <span className="text-xs text-blue-500">...see more</span>
        </p>
        <div>
          <PrimaryBtn
